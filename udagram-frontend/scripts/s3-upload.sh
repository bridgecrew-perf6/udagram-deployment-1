# Get start datetime:
start_time=`date +%s`

# Display message on deploy:
echo $'\n==> Start uploading script ...'

# change directory to udagram-frontend/:
cd ../

# Upload to S3:
echo $'==> Uploading static files ... This may take a while.\n'
aws s3 cp --recursive --acl public-read ./www s3://mokhtar-udagram-bucket/

# Play a beep sound after finising:
echo $'\n==> Finished uploading script ...'
echo -en '\007'

# Get end datetime:
end_time=`date +%s`

# Calculate runtime:
echo "==> Execution runtime was `expr $end_time - $start_time` s."

# Wait for any key to be pressed:
read -p "==> Press any key to close the terminal ... " -n1 -s
